...
var hud_y = 20;
var scale = 2; // 2x size
var icon_w = sprite_get_width(sWeaponsHUD) * scale;
var icon_h = sprite_get_height(sWeaponsHUD) * scale;
...
if (player.current_weapon_index >= 0 && player.current_weapon_index < array_length(player.weapons)) {
    ...
    if (frame >= sprite_get_number(sWeaponsHUD)) frame = 0; // safety check
    
    draw_sprite_ext(sWeaponsHUD, frame, hud_x, hud_y, scale, scale, 0, c_white, 1);
